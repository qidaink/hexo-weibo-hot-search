fetch('https://hexo-weibo-hot-search.vercel.app/api').then(data => data.json()).then(data => {
  ...
  for (let item of data) {
    html += '<div class="weibo-list-item"><div class="weibo-hotness ' + hotness[(item.hot || '荐')] + '">' + item.rank + '</div>'
      + '<span class="weibo-title"><a title="' + item.title + '"href="' + item.url + '" target="_blank" rel="external nofollow noreferrer">' + item.title + '</a></span>'
      + '<div class="weibo-num"><span>' + item.num + '</span></div>'
      + '<div class="weibo-hotness ' + hotness[(item.hot || '荐')] + '">' + (item.hot || '荐') + '</div></div>'
  }
  html += '</div>'
